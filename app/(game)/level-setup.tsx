...
export default function LevelSetupScreen() {
  ...
  const playerLevel =
    parseInt((params.playerLevel ?? String(user?.level ?? 1)) as string, 10) ||
    1;
  ...
  return (
    <SafeAreaView
      style={{ flex: 1, backgroundColor: isDark ? "#0f0f23" : "#ffffff" }}
    >
      <AppHeader />
      ...
      <View style={styles.body}>
        ...
        <Text
          style={[styles.currentLevel, { color: isDark ? "#ddd" : "#333" }]}
        >
          Текущий уровень:{" "}
          <Text style={{ fontWeight: "bold" }}>#{playerLevel}</Text>
        </Text>
